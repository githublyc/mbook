...
import (
	...
	"mbook/webook/pkg/grpcx/interceptor/trace"
	"net"
	"testing"
	"time"
)
...
func (s *InterceptorTestSuite) TestClient() {
	...
	cc, err := grpc.NewClient("localhost:8090",
		grpc.WithChainUnaryInterceptor(trace.
			NewOTELInterceptorBuilder("client_test",
...
func (s *InterceptorTestSuite) TestServer() {
	...
	server := grpc.NewServer(
		grpc.ChainUnaryInterceptor(
			NewLogInterceptor(t),
			trace.NewOTELInterceptorBuilder(
				"server_test", nil, nil).
	...
	//在服务端接入限流的另一种思路
	//RegisterUserServiceServer(server, &LimiterUserServiceServer{
