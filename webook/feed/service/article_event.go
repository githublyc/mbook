...
func (h *ArticleEventHandler) FindFeedEvents(ctx context.Context, uid, timestamp, limit int64) ([]domain.FeedEvent, error) {
	...
	eg.Go(func() error {
		...
		evts, err := h.repo.FindPullEventsWithTyp(ctx,
			ArticleEventName, followeeIDs, timestamp, limit)
		if err != nil {
			return err
		}
		lock.Lock()
	...
	eg.Go(func() error {
		evts, err := h.repo.FindPushEventsWithTyp(ctx,
			ArticleEventName, uid, timestamp, limit)
		if err != nil {
			return err
		}
		lock.Lock()
...
func (h *ArticleEventHandler) CreateFeedEvent(ctx context.Context, ext domain.ExtendFields) error {
	...
	resp, err := h.followClient.GetFollowStatic(ctx,
		&followv1.GetFollowStaticRequest{Followee: uid})
	if err != nil {
		return err
	}
	...
	if resp.FollowStatic.Followers > threshold {
		...
		events := slice.Map(fresp.FollowRelations,
