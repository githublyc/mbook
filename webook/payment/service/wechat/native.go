...
import (
	...
	"mbook/webook/payment/domain"
	"mbook/webook/payment/events"
	"mbook/webook/payment/repository"
...
func (n *NativePaymentService) Prepay(ctx context.Context,
	pmt domain.Payment) (string, error) {
...
func (n *NativePaymentService) SyncWechatInfo(ctx context.Context,
	...
	txn, _, err := n.svc.QueryOrderByOutTradeNo(ctx,
...
func (n *NativePaymentService) HandleCallback(ctx context.Context,
	transaction *payments.Transaction) error {
	return n.updateByTxn(ctx, transaction)
}

func (n *NativePaymentService) updateByTxn(ctx context.Context,
	transaction *payments.Transaction) error {
	status, ok := n.nativeCBTypeToStatus[*transaction.TradeState]
	if !ok {
		return fmt.Errorf("%w, 微信的状态是 %s", errUnknownTransactionState,
	...
	err := n.repo.UpdatePayment(ctx, domain.Payment{
		...
		BizTradeNO: *transaction.OutTradeNo,
		Status:     status,
	})
	if err != nil {
		return err
