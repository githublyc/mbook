...
func (dao *GORMTagDAO) CreateTagBiz(ctx context.Context,
	tagBiz []TagBiz) error {
	if len(tagBiz) == 0 {
		return nil
	}
	...
	return dao.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		first := tagBiz[0]
		err := tx.Model(&TagBiz{}).
			Delete("uid = ? AND biz = ? AND biz_id = ?",
				first.Uid, first.Biz, first.BizId).Error
		if err != nil {
			return err
		}
...
func (dao *GORMTagDAO) GetTagsByBiz(ctx context.Context, uid int64, biz string, bizId int64) ([]Tag, error) {
	...
	err := dao.db.WithContext(ctx).Model(&TagBiz{}).
		...
		// tag_bizs.uid
		Where("Tag.uid = ? AND biz = ? AND biz_id = ?",
			uid, biz, bizId).Find(&tagBizs).Error
	if err != nil {
		return nil, err
	}
