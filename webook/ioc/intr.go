...
func InitIntrClient(svc service.InteractiveService) intrv1.InteractiveServiceClient {
	...
	viper.OnConfigChange(func(e fsnotify.Event) {
		cfg = Config{}
		er := viper.UnmarshalKey("grpc.client.intr", &cfg)
		if er != nil {
			// 这边做不了什么
			panic(er)
		}
...
func InitIntrClientV1(client *etcdv3.Client) intrv1.InteractiveServiceClient {
	...
	resolver1, err := resolver.NewBuilder(client)
	if err != nil {
		panic(err)
	}
	opts := []grpc.DialOption{grpc.WithResolvers(resolver1)}
	if !cfg.Secure {
		opts = append(opts, grpc.WithTransportCredentials(insecure.NewCredentials()))
	}
	cc, err := grpc.NewClient(cfg.Addr, opts...)
