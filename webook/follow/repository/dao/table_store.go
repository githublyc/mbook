...
func (t *TableStoreFollowRelationDao) UpdateStatus(ctx context.Context,
	followee int64, follower int64, status uint8) error {
	cond := tablestore.NewCompositeColumnCondition(
		tablestore.LO_AND)
	cond.AddFilter(tablestore.NewSingleColumnCondition(
		"follower", tablestore.CT_EQUAL, follower))
	cond.AddFilter(tablestore.NewSingleColumnCondition(
		"followee", tablestore.CT_EQUAL, followee))
...
func (t *TableStoreFollowRelationDao) CreateFollowRelation(ctx context.Context,
	c FollowRelation) error {
	// 创建关注关系
	// 你需要的是 insert or update 语义
	...
	now := time.Now()
	change.PutColumn("status",
		int64(FollowRelationStatusActive))
