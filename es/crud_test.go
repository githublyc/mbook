...
func (s *ElasticSearchTestSuite) SetupSuite() {
	...
	s.es = es
	ol, err := olivere.NewClient(olivere.
...
`
	...
	_, err = s.olivere.CreateIndex("user_idx_go").Body(def).Do(ctx)
	require.NoError(s.T(), err)
}
...
`
	_, err := s.es.Index("user_idx_go",
		strings.NewReader(data), s.es.Index.WithContext(ctx))
	require.NoError(s.T(), err)
...
`
	...
	resp, err := s.olivere.Search("user_idx_go").
		Query(olQuery).Do(ctx)
	require.NoError(s.T(), err)
